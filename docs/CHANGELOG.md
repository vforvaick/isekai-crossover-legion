...
- **Convex Migration**: Executed `markDormant` to correctly flag dormant agents in the UI.
- **VPS Deployment Sync**: Resolved divergent branches on `fight-uno` and synchronized workspace with latest GitHub `main`.
- **Verified Features**:
  - Full Task CRUD (Create/Edit/Delete) confirmed functional.
  - Kanban Domain Filtering (Office/Trading/Personal/Deployment) confirmed functional.
  - Agent Detail Pages (`/agents/[handle]`) correctly linked and populated.
  - Heartbeat Visualization (live pulse indicators) confirmed functional.

### Changed
- `convex/seed.ts`: Updated default statuses for Rimuru, Senku, and Demiurge to "idle" (active).
- `README.md`: Roster table updated to match reference specification.
- `/docs`: `ROADMAP.md` and `HANDOVER_PLAN.md` updated to reflect current progress.
